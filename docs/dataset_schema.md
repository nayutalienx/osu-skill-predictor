...
1. Fetch a pool of candidate users from rankings for `osu`.
2. For each user, fetch the current user profile snapshot.
3. For each user, fetch recent scores with failures included.
4. For each beatmap referenced by those scores, fetch beatmap metadata.
5. Optionally fetch beatmap attributes for specific mod combinations if needed.
6. Flatten the result into one training row per score.
...
After this schema is accepted, the next implementation item should be:

- create a tiny sample dataset under `data/sample/`
- with a handful of rows that follow this exact schema
- so feature engineering and API contracts can be developed locally before larger-scale collection
